...
void ChangesManager::writeChange(std::string newFileState)
{
    ...
    change.before = std::string(mismatched_range_begin.first, currentFileState.begin() + end_pos_before);// in accorfance to our example it'll be ""
    change.after = std::string(mismatched_range_begin.second, newFileState.begin() + end_position_after);// in accorfance to our example it'll be "00"
...
std::string ChangesManager::returnToPreviousState()// replace text and return previous version
{
    if(!changesHistory.size())//if is no any record in the change history list
